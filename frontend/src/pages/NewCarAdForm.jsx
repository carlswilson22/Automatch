...
const NewCarAdForm = () => {
  ...
  const scanCarPhotoAi = async (imageSrc) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPhotoAiScanStatus('done');
      } else {
        setPhotoAiScanStatus('error');
      }
    } catch (err) {
      setPhotoAiScanStatus('error');
    }
  ...
  const handleSubmit = (e) => {
    ...
    const numericPrice = Number(formData.preco.replace(/[^0-9,-]+/g,"").replace(",", ".")) || 0;

    const newCarRecord = addNewCar({
                  ...
                  <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-100 text-sm">
                        {stores.find(s => s.id?.toString() === formData.store_id?.toString())?.name || 'Automatch Vitrine'}
                      </p>
                    </div>
                  </div>
            ...
            <div className="space-y-3 pt-6 border-t border-slate-100">
              ...
              {!formData.imagem ? (
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {/* Botão de Perícia Visual IA da Capa */}
                  <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-bold text-slate-800">Perícia Pericial de Avarias por IA</p>
                        <p className="text-[11px] text-slate-500">Analise a imagem para confirmar integridade da lataria antes de publicar.</p>
                      </div>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      {photoAiScanStatus === 'scanning' ? (
                        <>
                          ...
                          <span>Periciando...</span>
                        </>
                      ) : (
                        <>
                          <Scan className="w-3.5 h-3.5" />
                          <span>Verificar Avarias com IA</span>
                        </>
                      )}
                    </button>
                  </div>

                  {/* Resultado da Perícia IA */}
                  <AnimatePresence>
                    {photoAiResult && (
                      <motion.div
                        initial={{ opacity: 0, height: 0 }}
                        animate={{ opacity: 1, height: 'auto' }}
                        className="p-4 bg-slate-900 border border-slate-800 rounded-xl text-white space-y-2"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            {photoAiResult.tem_avarias ? (
                              <span className="text-amber-400 font-bold text-xs flex items-center gap-1">
                                <AlertTriangle className="w-4 h-4" /> Avarias Detectadas
                              </span>
                            ) : (
                              <span className="text-emerald-400 font-bold text-xs flex items-center gap-1">
                                <CheckCircle2 className="w-4 h-4" /> Lataria 100% Íntegra
                              </span>
                            )}
                            <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-slate-400">
                              Score: {photoAiResult.score_lataria}%
                            </span>
                          </div>
                          <button type="button" onClick={() => setPhotoAiResult(null)} className="text-slate-400 hover:text-white text-xs">
                            <X className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <p className="text-xs text-slate-300 leading-relaxed">
                          {photoAiResult.resposta}
                        </p>
                      </motion.div>
                    )}
                  </AnimatePresence>
